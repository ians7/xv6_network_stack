...
extern uint64 uvmthreaded_alloc(struct proc*, uint64, uint64, uint64);
extern uint64 uvmthreaded_dealloc(struct proc*, uint64, uint64);
...
// currently just an exact copy of exit()
...
int create_thread(void* (*fn_addr)(void *), void *args, void *stack_addr, void (*exit_fn)(uint64)) {
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Set location of new thread to the start of its function, stack to user-allocated memory location,
  // and a0 to the address of the function arguments
...
uint64 join_thread(uint64 thread_id, uint64 status_addr) {
  ...
  for (thread_idx = 0; thread_idx < MAX_THREADS; thread_idx++) {
    if (p->infant_threads[thread_idx] && thread_id == p->infant_threads[thread_idx]->pid) {
      child = p->infant_threads[thread_idx];
      break;
    }
  }
